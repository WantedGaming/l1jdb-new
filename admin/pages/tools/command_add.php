...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // If no errors, insert the new command
    if (empty($errors)) {
        $insert_sql = "INSERT INTO commands (name, access_level, class_name, description) VALUES (?, ?, ?, ?)";
        $insert_stmt = $conn->prepare($insert_sql);
        $insert_stmt->bind_param("siss", $name, $access_level, $class_name, $description);
        
        if ($insert_stmt->execute()) {
            $_SESSION['message'] = "Command added successfully!";
            $_SESSION['message_type'] = "success";
            header("Location: gm_commands.php");
            exit;
        } else {
            $errors[] = "Error adding command: " . $conn->error;
        }
    }
}
?>
                        ...
                        <form method="POST" action="">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="access_level" name="access_level" required>
                                    <option value="0" <?= isset($_POST['access_level']) && $_POST['access_level'] == 0 ? 'selected' : '' ?>>Admin (0)</option>
                                    <option value="50" <?= isset($_POST['access_level']) && $_POST['access_level'] == 50 ? 'selected' : '' ?>>GM (50)</option>
                                    <option value="200" <?= isset($_POST['access_level']) && $_POST['access_level'] == 200 ? 'selected' : '' ?>>Mod (200)</option>
                                    <option value="9999" <?= isset($_POST['access_level']) && $_POST['access_level'] == 9999 ? 'selected' : '' ?>>Player (9999)</option>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="class_name" name="class_name" 
                                       value="<?= htmlspecialchars($_POST['class_name'] ?? '') ?>" required>
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="submit" class="btn btn-accent">Add Command</button>
                            </div>
                        </form>
                    </div>
                </div>
