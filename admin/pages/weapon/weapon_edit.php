    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-10 text-center hero-content">
                ...
                <?php 
                // Check if weapon is in droplist
                ...
                if ($droplist_result && $droplist_result->num_rows > 0) {
                    ...
                    if ($droplist_count > 0) {
                        echo '<div class="badge bg-accent mt-2">Used in ' . $droplist_count . ' monster drop tables</div>';
                    }
                }
                
                // Check if weapon has skills
                $check_skills_sql = "SELECT COUNT(*) as skill_count FROM weapon_skill WHERE weapon_id = $weapon_id";
                $skills_result = $conn->query($check_skills_sql);
                
                if ($skills_result && $skills_result->num_rows > 0) {
                    $skills_row = $skills_result->fetch_assoc();
                    $skill_count = $skills_row['skill_count'];
                    
                    if ($skill_count > 0) {
                        echo '<div class="badge bg-info mt-2 ms-2">Has ' . $skill_count . ' weapon skills</div>';
                    }
                }
                ?>
            </div>
